...
def convert_to_phen(phenotypes_df, path, include_header=False):
  ...
  df_phen = phenotypes_df[['individual_id', 'phenotype']].rename(columns={'individual_id': 'person_id', 'phenotype': 'phenotypes'})

  df_phen.to_csv(path, sep='\t', index=False, header=include_header)

def sim_phenotypes(grg, model, num_causal, random_seed, normalize_genetic_values_before_noise=False, noise_heritability=None, user_mean=None, user_cov=None, normalize_genetic_values_after=False, save_effect_output=False, effect_path=None, standardized_output=False, path=None, header=False):
  """
  ...
  causal_mutation_df = sim_grg_causal_mutation(grg, num_causal=num_causal, model=model, random_seed=random_seed)
  ...
  if noise_heritability is not None:
    ...
    if check:
      phenotypes = sim_env_noise(individual_genetic_values, user_defined=True, mean=user_mean, std=user_cov)
    else:
      phenotypes = sim_env_noise(individual_genetic_values, user_defined=True, means=user_mean, cov=user_cov)

    final_phenotypes = normalize(phenotypes, normalize_genetic_values=normalize_genetic_values_after)
...
def sim_phenotypes_custom(grg, input_effects, normalize_genetic_values_before_noise=False, noise_heritability=None, user_mean=None, user_cov=None, normalize_genetic_values_after=False, save_effect_output=False, effect_path=None, standardized_output=False, path=None, header=False):
  """
  Function to simulate phenotypes in one go by combining all intermittent stages.
  ...
  if isinstance(input_effects, dict):
    causal_mutation_df = pd.DataFrame(list(input_effects.items()), columns=['mutation_id', 'effect_size'])
