...
def normalize_genetic_val(phenotype_df):
    ...
    phenotype_df['genetic_value'] = (phenotype_df['genetic_value'] - mean_df) / std_dev_df

    return phenotype_df

def normalize_individual_phenotypes(phenotype_df, h2_curr, user_noise, normalize_genetic_values):
  """
  ...
  if((h2_curr!=1 and user_noise==False) or user_noise==True):
    ...
    phenotype_df['genetic_value'] = (phenotype_df['genetic_value'] - old_genetic_mean) / old_std
    phenotype_df['environmental_noise'] = (phenotype_df['environmental_noise'] - old_noise_mean) / old_std
...
def normalize(phenotypes, normalize_genetic_values=False):
  ...
  if len(causal_mutation_id_check)==1:
    normalized_phenotype_df = normalize_individual_phenotypes(phenotype_df, h2[0], user_noise, normalize_genetic_values)
     
  else:
    df_dict = {k: v.sort_values('individual_id') for k, v in phenotype_df.groupby('causal_mutation_id')}
    ...
    for i in range(num):
      ...
      normalized_df = normalize_individual_phenotypes(pheno_df, h2[i], user_noise, normalize_genetic_values)
  ...
  normalized_phenotype_df['individual_id'] = normalized_phenotype_df['individual_id'].astype(int)
  normalized_phenotype_df['causal_mutation_id'] = normalized_phenotype_df['causal_mutation_id'].astype(int)
...
def quantile_normalize_to_normal(column, epsilon=1e-10):
    ...
    ranks = ranks.clip(lower=epsilon, upper=1 - epsilon)  # adjust ranks to avoid 0 and 1
...
def quantile_normalize(phenotype_df, phenotype_normalize=True, normalize_both=False):
  ...
  if len(causal_mutation_id_check)==1:
    if normalize_both==True:
      phenotype_df["normalized_genetic_value"] = quantile_normalize_to_normal(phenotype_df["genetic_value"])
      phenotype_df["normalized_phenotype"] = quantile_normalize_to_normal(phenotype_df["phenotype"])
    ...
    quantile_normalized_phenotype_df = pd.concat(interleaved_rows, axis=1).transpose()

    quantile_normalized_phenotype_df.reset_index(drop=True, inplace=True)

    quantile_normalized_phenotype_df['individual_id'] = quantile_normalized_phenotype_df['individual_id'].astype(int)
    quantile_normalized_phenotype_df['causal_mutation_id'] = quantile_normalized_phenotype_df['causal_mutation_id'].astype(int)
