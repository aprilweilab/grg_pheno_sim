...
class GRGCausalMutationSimulator:
    ...
    def run(self):
        ...
        causal_mutation_data = {
            ...
            "causal_mutation_id": np.tile(np.arange(num_traits), len(self.causal_sites))
        }
        df = pd.DataFrame(causal_mutation_data)
        
        # sort by mutation_id first and then by causal_mutation_id to ensure the order
        df = df.sort_values(by=["mutation_id", "causal_mutation_id"]).reset_index(drop=True)
        
        return df


def sim_grg_causal_mutation(grg, model, *, num_causal=None, causal_sites=None, random_seed=None):
    """
...
def allele_frequencies(grg, effects):
    ...
    if effects['causal_mutation_id'].sum() == 0: #base case for univariate causal mutation simulation
        ...
        df_dict = {k: v.sort_values('mutation_id') for k, v in effects.groupby('causal_mutation_id')}
...
def additive_sim_grg_causal_mutation(grg, causal_mutation_df, causal_id):
    ...
    for _, row in causal_mutation_df.iterrows():
        ...
        if mutation_idx < num_mutations:  # Add safety check
            ...
            raise ValueError(f"Mutation ID {mutation_idx} exceeds the number of mutations in GRG ({num_mutations})")
    
    # Use dot_product with DOWN direction to get effect sizes for all nodes
    sample_effects = pygrgl.dot_product(
        grg=grg,
        input=mutation_vector,
...
def additive_effect_sizes(grg, causal_mutation_df):
    ...
    """
    if causal_mutation_df['causal_mutation_id'].sum() == 0: #base case for univariate causal mutation simulation
        samples_effect_sizes = additive_sim_grg_causal_mutation(grg, causal_mutation_df, 0)

        return samples_effect_sizes
     
    else:  #for multivariate causal mutation simulation
        df_dict = {k: v.sort_values('mutation_id') for k, v in causal_mutation_df.groupby('causal_mutation_id')}
        ...
        samples_effect_sizes['sample_node_id'] = samples_effect_sizes['sample_node_id'].astype(int)
        samples_effect_sizes['causal_mutation_id'] = samples_effect_sizes['causal_mutation_id'].astype(int)
...
def singular_samples_to_individuals(additive_causal_mutation_df):
    ...
    individual_df = {
        ...
        'genetic_value': additive_causal_mutation_df['genetic_value'].groupby(additive_causal_mutation_df.index // 2).sum(),
        'causal_mutation_id': additive_causal_mutation_df['causal_mutation_id'][::2].values
    }
...
def multiple_samples_to_individuals(additive_causal_mutation_df):
    ...
    """
    additive_causal_mutation_df['pair_id'] = additive_causal_mutation_df['sample_node_id'] // 2

    individual_df = additive_causal_mutation_df.groupby('pair_id').agg({
        'genetic_value': 'sum',  
        'causal_mutation_id': 'first'  
    }).reset_index()
...
def samples_to_individuals(additive_causal_mutation_df):
    ...
    """
    if additive_causal_mutation_df['causal_mutation_id'].sum() == 0: #base case for univariate causal mutation
        ...
        df_dict = {k: v.sort_values('sample_node_id') for k, v in additive_causal_mutation_df.groupby('causal_mutation_id')}
        ...
        individuals['causal_mutation_id'] = individuals['causal_mutation_id'].astype(int)
...
def normalize_individual_genetic_values(individual_df, mean=0, var=1):
    ...
    individual_df['genetic_value'] = (individual_df['genetic_value'] - mean_df) / std_dev_df
...
def normalize_genetic_values(individuals_df, mean=0, var=1):
    ...
    if individuals_df['causal_mutation_id'].sum() == 0: #base case for univariate causal mutation
        individuals_df = normalize_individual_genetic_values(individuals_df, mean, var)

        return individuals_df

    else:
        df_dict = {k: v.sort_values('individual_id') for k, v in individuals_df.groupby('causal_mutation_id')}
        ...
        individuals_df['causal_mutation_id'] = individuals_df['causal_mutation_id'].astype(int)
