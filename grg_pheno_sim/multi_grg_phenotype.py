...
def intermediate_genetic_vals(grg, model, num_causal, random_seed, normalize_genetic_values_before_noise, grg_name, effect_path):
    """
...
def sim_phenotypes_multi_grg_ram(
    ...
    for grg in all_grgs:
        ...
        genetic_val_df = intermediate_genetic_vals(grg, model, num_causal_per_file, np.random.seed(random_seed), normalize_genetic_values_before_noise, grg_name, effect_path=path)
    ...
    combined_genetic_df = combined_genetic_df[['individual_id', 'genetic_value', 'causal_mutation_id']]
    ...
    if noise_heritability is not None:
        ...
        if check:
            phenotypes = sim_env_noise(combined_genetic_df, user_defined=True, mean=user_mean, std=user_cov)
        else:
            phenotypes = sim_env_noise(combined_genetic_df, user_defined=True, means=user_mean, cov=user_cov)
...
def sim_phenotypes_multi_grg_sequential(
    ...
    for grg_file in grg_files:
        ...
        genetic_val_df = intermediate_genetic_vals(temp_grg, model, num_causal_per_file, np.random.seed(random_seed), normalize_genetic_values_before_noise, grg_file, effect_path=path)
        all_genetic_values.append(genetic_val_df)
