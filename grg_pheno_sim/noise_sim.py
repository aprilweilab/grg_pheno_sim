...
class GrgEnvSimulator:
    ...
    def __init__(self, genetic_value_df, h2, random_seed):
        self.genetic_value_df = genetic_value_df[["causal_mutation_id", "individual_id", "genetic_value"]]
        self.h2 = h2
    ...
    def sim_environment_multivariate(self, num_causal, means=None, cov=None, tailored=False):
        """
...
def sim_env_noise(genetic_value_df, *, h2=None, random_seed=None, user_defined=False, mean=None, std=None, means=None, cov=None):
    """
    ...
    if np.min(causal_mutation_id) != 0 or np.max(causal_mutation_id) != len(causal_mutation_id) - 1:
    ...
    if len(causal_mutation_id)==1:
      ...
      if user_defined==True:
        phenotype_df = simulator.sim_environment_univariate(tailored=user_defined, user_mean=mean, user_std=std)
      else:
      ...
      if user_defined==True:
        phenotype_df = simulator.sim_environment_multivariate(tailored=user_defined, means=means, cov=cov, num_causal=num_causal)
      else:
